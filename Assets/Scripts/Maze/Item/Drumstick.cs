    ...
    public class Drumstick : MazeItem
    {
        ...
        protected override void EnterEffect()
        {
            ...
            size.position = new Vector3(
                size.position.x + size.sizeDelta.x * (hungerExtensionEffect / (2 * CoreBars.HealthCore.MaxValue)),
                size.position.y, size.position.z);

            size.localScale = new Vector3(
                size.localScale.x * ((hungerExtensionEffect / CoreBars.HungerCore.MaxValue) + 1), size.localScale.y,
                size.localScale.z);
